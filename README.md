...
基于 **.NET 8 + Vue 3** 的进销存管理系统，采用三层架构，集成 Redis 缓存、RabbitMQ 消息队列与 Docker 容器化部署，覆盖商品、分类、供应商、客户、入库、出库、库存、仪表盘等核心业务流程，配备 53 个单元测试与 GitHub Actions CI 流水线。
...
| FluentValidation | 请求参数校验（6 个验证器） |
...
| Pinia 3 | 用户认证状态管理（token 持久化至 localStorage） |
...
- **Docker Compose**：五容器编排（SQL Server / Redis / RabbitMQ / API / Web），含健康检查与资源限制
...
│   │   ├── Validators/                       #   6 个 FluentValidation 验证器
...
| 项目 | 职责 |
|---|---|
| `WiseWMS.Api` | 接收 HTTP 请求、认证授权、参数绑定、返回响应、后台服务 |
...
### 5. RabbitMQ 消息队列与缓存一致性

入库/出库操作事务提交后，通过 `MessagePublisher` 发布库存变动消息至 `inventory_sync` 队列。`InventorySyncConsumer` 后台服务消费消息，清除 Redis 缓存键 `products_all` 和 `dashboard_stats`，实现缓存与数据库的最终一致性。
...
- `FluentValidation` 做复杂规则校验（6 个验证器，覆盖供应商/客户/入库/出库等场景）
...
- 使用 Pinia 管理用户认证状态（token、displayName、role）
- 登录/登出 action 同步写入 localStorage 做持久化
- 路由守卫从 Pinia 读取 token 判断登录态
- 业务视图通过 `userStore.role` 控制 Admin 权限按钮显隐
...
> **注意**：`InventorySyncConsumer` 当前硬编码使用 `localhost` 连接 RabbitMQ。在 Docker 环境中若出现连接问题，请将其改为从 `ConnectionStrings__RabbitMQ` 配置读取（对应 Docker Compose 服务名 `rabbitmq`）。
...
# 配置 JWT 密钥（Linux/macOS）
export Jwt__Key="YourSuperSecretKey_MustBeAtLeast32Chars!"

# 配置 JWT 密钥（Windows PowerShell）
$env:Jwt__Key="YourSuperSecretKey_MustBeAtLeast32Chars!"
...
| 商品 | GET | `/api/Products/all` | 全量商品列表 |
...
| 供应商 | GET | `/api/Suppliers/all` | 全量供应商列表 |
...
| 客户 | GET | `/api/Customers/all` | 全量客户列表 |
| 客户 | GET | `/api/Customers/{id}` | 客户详情 |
| 客户 | POST | `/api/Customers` | 新增客户 |
| 客户 | PUT | `/api/Customers/{id}` | 修改客户 |
| 客户 | DELETE | `/api/Customers/{id}` | 删除客户（Admin） |
| 入库 | GET | `/api/Inbound?keyword=&page=&pageSize=` | 入库单列表 |
| 入库 | GET | `/api/Inbound/{id}` | 入库单详情（含明细） |
| 入库 | POST | `/api/Inbound` | 新建入库单 |
| 出库 | GET | `/api/Outbound?keyword=&page=&pageSize=` | 出库单列表 |
| 出库 | GET | `/api/Outbound/{id}` | 出库单详情（含明细） |
| 出库 | POST | `/api/Outbound` | 新建出库单 |
| 库存 | GET | `/api/Inventory/transactions/{productId}` | 库存流水 |
...
53 个单元测试覆盖全部 9 个业务服务，使用 xUnit + Moq + EF Core InMemory：
...
| CategoryServiceTests | 7 | 列表、新增、重复校验、删除（含引用检查） |
...
> 覆盖率：Application 层行覆盖约 78%；整体覆盖受 API 层/中间件/后台服务未纳入单元测试影响。
...
### Q6：`dotnet test` 提示 AutoMapper 有安全漏洞？

当前引用的 `AutoMapper 13.0.1` 存在已知高危漏洞。建议升级至 14.x 或官方修复版本，并在升级后运行完整测试验证映射行为。
